...
struct dvb_usb_device *d = i2c_get_adapdata(adap);
	...
	case 2: {
		...
		tbs5922se_op_rw(d->udev, 0x90, 0, 0,
					obuf, 3, TBS5922SE_WRITE_MSG);
		msleep(5);
		tbs5922se_op_rw(d->udev, 0x91, 0, 0,
					ibuf, 1, TBS5922SE_READ_MSG);
	...
	case 1:
		...
		case 0x68: {
				...
				tbs5922se_op_rw(d->udev, 0x80, 0, 0,
							buf6, msg[0].len+2, TBS5922SE_WRITE_MSG);
				//msleep(3);
			break;
		}
		case 0x63: {
...
static struct tbs5922se_config tbs5923_fe_config = {
	.tbs5922se_address = 0x68,

	.tbs5922se_ctrl = tbs5922ctrl,
};
...
static int tbs5922se_frontend_attach(struct dvb_usb_adapter *d)
{
	u8 buf[20];
        ...
        if ((d->fe[0] = dvb_attach(tbs5922se_attach, &tbs5923_fe_config,
                                        &d->dev->i2c_adap)) != NULL) {
			printk("TBS5922SE attached.\n");
			dvb_attach(tbsfe_attach, d->fe[0]);

			buf[0] = 7;
			buf[1] = 1;
			tbs5922se_op_rw(d->dev->udev, 0x8a, 0, 0,
					buf, 2, TBS5922SE_WRITE_MSG);
...
MODULE_DESCRIPTION("TurboSight TBS 5922SE driver");
MODULE_VERSION("1.0");
MODULE_LICENSE("GPL");
