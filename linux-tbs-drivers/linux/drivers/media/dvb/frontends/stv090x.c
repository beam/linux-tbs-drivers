...
/* define how issy is done */
static int issy = 1;
module_param(issy, int, 0644);
MODULE_PARM_DESC(issy, "0: issy is disabled, "\
	"1: issy is enabled(default:1)");
...
static enum dvbfe_search stv090x_search(struct dvb_frontend *fe, struct dvb_frontend_parameters *p)
{
	...
	if (!issy)
		stv090x_write_reg(state, STV090x_P1_TSSTATEM, \
		stv090x_read_reg(state, STV090x_P1_TSSTATEM) | 0x01);
	else 
		stv090x_write_reg(state, STV090x_P1_TSSTATEM, \
		stv090x_read_reg(state, STV090x_P1_TSSTATEM) & ~0x01);
	...
	case STV090x_DVBS2:
		...
		if (lock_f) {
			...
			*cnr = 0xFFFF - ((val * 0xFFFF) / div);
			...
			if (esno) {
				div = stv090x_table_lookup(stv090x_s2cn_tab, last, val);
				*cnr = div;
			}
		}
		break;
	...
	case STV090x_DSS:
		...
		if (lock_f) {
			...
			*cnr = 0xFFFF - ((val * 0xFFFF) / div);
			...
			if (esno) {
				div = stv090x_table_lookup(stv090x_s1cn_tab, last, val);
				*cnr = div;
			}
		}
		break;
