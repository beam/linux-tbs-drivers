...
static int do_tune(int fefd, unsigned int ifreq, unsigned int sr, enum fe_delivery_system delsys,
		   int modulation, int fec, int rolloff, int mis)
{
...
int zap_to(unsigned int adapter, unsigned int frontend, unsigned int demux,
	   ...
	   int modulation, int fec, int rolloff, int mis, int human_readable)
{
	struct dtv_property p[] = {
		{ .cmd = DTV_CLEAR },
	};
	...
	if (diseqc(fefd, sat_no, pol, hiband))
		if (do_tune(fefd, ifreq, sr, delivery, modulation, fec, rolloff, mis))
...
again:
	...
	while (!feof(cfp)) {
		...
		if (params_debug){
			printf("rolloff 0x%x\n"
				"mis %i, vpid 0x%04x, apid 0x%04x, sid 0x%04x\n", rolloff, mis, vpid, apid, sid);
		} else {
			field = NULL;
			map_to_user(rolloff, rolloff_values, &field);
			printf("rolloff %s\n"
				"mis %i, vpid 0x%04x, apid 0x%04x, sid 0x%04x\n", field, mis, vpid, apid, sid);
		}

		ret = zap_to(adapter, frontend, demux, sat_no, freq * 1000,
				pol, sr, vpid, apid, sid, dvr, rec_psi, bypass,
				delsys, modulation, fec, rolloff, mis, human_readable);
...
int main(int argc, char *argv[])
{
	...
	if (!read_channels(chanfile, list_channels, chan_no, chan_name,
	    ...
	    modulation, fec, rolloff, mis, human_readable, params_debug, use_vdr_format))
